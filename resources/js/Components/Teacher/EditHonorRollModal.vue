...
const submit = () => {
  ...
  form.put(route('honorroll.update', props.record.id), {
    ...
    onSuccess: () => {
      Swal.fire({
        title: 'Honor Roll Updated!',
        text: 'The honor roll record has been successfully updated.',
        icon: 'success',
        background: '#1f2937',
        color: '#f9fafb',
        backdrop: 'rgba(0, 0, 0, 0.7)',
    ...
    onError: (errors) => {
      Swal.fire({
        title: 'Update Failed!',
        text: 'Failed to update honor roll record. Please check your input and try again.',
        icon: 'error',
        background: '#1f2937',
        color: '#f9fafb',
        backdrop: 'rgba(0, 0, 0, 0.7)',
